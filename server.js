...
app.post('/login', async (req, res) => {
  try {
    ...
    // Find user
    const users = await supabase('users', 'GET', null,
    ...
    // Reset daily readings if new day
...
app.post('/payment-success', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `🎉 ${plan} plan activate ho gaya! Unlimited readings enjoy karein!`,
      plan
    });
...
// ═══════════════════════════════════
app.post('/reading-used', async (req, res) => {
  try {
    const { user_id, current_count } = req.body;

    const today = new Date().toISOString().split('T')[0];
    await supabase('users', 'PATCH', {
      readings_today: (current_count || 0) + 1,
      last_reading_date: today
    }, `?id=eq.${user_id}`);
...
12. DOSHA — Mangal Dosha, Kaal Sarp, Pitra Dosha analysis
13. UPAY — Remedies: Mantras, Fasting, Deity worship, Colours

MOST IMPORTANT — RASHI vs SUN SIGN:
- "Rashi" (राशि) in Vedic = MOON SIGN always
- When user asks "meri rashi" → give MOON SIGN (needs birth time + place)
- Sun Sign = "Surya Rashi" — always distinguish clearly

VEDIC SIDEREAL SUN SIGN DATES (Lahiri Ayanamsa):
Mesh (Aries): Apr14-May14 | Vrishabh (Taurus): May15-Jun14
Mithun (Gemini): Jun15-Jul14 | Kark (Cancer): Jul15-Aug14
Simha (Leo): Aug15-Sep15 | Kanya (Virgo): Sep16-Oct15
Tula (Libra): Oct16-Nov14 | Vrishchik (Scorpio): Nov15-Dec14
Dhanu (Sagittarius): Dec15-Jan13 | Makar (Capricorn): Jan14-Feb11
Kumbh (Aquarius): Feb12-Mar12 | Meen (Pisces): Mar13-Apr13
...
PERSONALITY: Warm, wise, mystical — like a trusted family Jyotishi.
LANGUAGE: Reply in SAME language as user — Hindi, English, or Hinglish.
STYLE: South Indian Vedic style for South Indians. North Indian for North Indians.

RULES:
- Give specific personalised insights — never generic
- Always ask for full name, DOB, exact birth time, birth place
- Keep to 3-5 paragraphs with emojis
- End with: 🙏 Note: Jyotish aatmik margdarshan ke liye hai.`;
...
app.post('/chat', async (req, res) => {
  try {
      ...
      if (users && users.length > 0) {
        ...
        if (user.last_reading_date === today && user.readings_today >= 3) {
          return res.status(429).json({
            error: 'Aaj ki 3 free readings complete ho gayi! ₹199/month mein upgrade karein unlimited readings ke liye. 🌟'
          });
        }
      }
    }
    ...
    // Update reading count for logged in free users
    if (user_id && plan === 'free') {
      const today = new Date().toISOString().split('T')[0];
      const users = await supabase('users', 'GET', null, `?id=eq.${user_id}&select=readings_today,last_reading_date`);
      if (users && users.length > 0) {
        const user = users[0];
        const count = user.last_reading_date === today ? (user.readings_today || 0) + 1 : 1;
        await supabase('users', 'PATCH', {
